...
export const calculateWalkingTimeInMinutes = (
  distanceInMeters: number
): number => {
  const walkingSpeed: number = 80;
  if (distanceInMeters <= 0)
    throw new Error("0より大きい数値を入力してください。");
...
export const showResultBloodTypeTest = async (
  bloodTypeId: number
): Promise<string> => {
  const fetchResultBloodTypeApi: FetchResultBloodTypeApi =
    new FetchResultBloodTypeApi();
  const fetchData: BloodTypeTest =
    await fetchResultBloodTypeApi.getResultBloodTypeTest(bloodTypeId);
